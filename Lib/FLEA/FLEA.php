...
class FLEA
{
    ...
    function runMVC()
    {
        ...
        // 当地址改变的时候自动跳转到登录界面
        if($_SESSION['USERNAME'] != '' && strtolower($_SESSION['PHP_SELF']) != strtolower($_SERVER['PHP_SELF'])){
            session_destroy();
            redirect(url("Index"));
            exit;
        }
    ...
    class FLEA_Exception extends Exception
    {
        function FLEA_Exception($message = '', $code = 0)
        {
			...
			if(defined('ERROR_SYS_DIR')){
				...
				if($code=='7335941')$error=new TMIS_ErrorSys($message,'数据保存失败');
				else $error=new TMIS_ErrorSys($message);
				$error->jumpToPage();
			}
            //end
