...
<Workbook xmlns="urn:schemas-microsoft-com:office:spreadsheet"
 ...
 <Worksheet ss:Name="Sheet1">
  <Table x:FullColumns="1"
   x:FullRows="1" ss:DefaultColumnWidth="85" ss:DefaultRowHeight="15">
   <Row ss:AutoFitHeight="0" ss:Height="25.5">
   {assign var=cross value=$arr_field_info|@count}
    <Cell {if $cross>1}ss:MergeAcross="{$cross-1}"{/if} ss:StyleID="s24"><Data ss:Type="String">常州金马劲飞布业纺织有限公司客户往来余额对账单</Data></Cell>
   </Row>
   <Row ss:AutoFitHeight="0" ss:Height="25.5">
   {assign var=cross value=$arr_field_info|@count}
    <Cell {if $cross>1}ss:MergeAcross="{$cross-1}"{/if} ss:StyleID="s30"><Data ss:Type="String">客户：{$title1}</Data></Cell>
   </Row>
    <Row >
   {assign var=cross value=$arr_field_info|@count}
    <Cell {if $cross>1}ss:MergeAcross="{$cross-1}"{/if} ss:StyleID="s29"><Data ss:Type="String"><font style="font-size:12px;">截止{$date}，贵单位尚欠我公司货款合计人民币：{$flag}{$moneyJieyuCap}（{$moneyJieyu}元）,如有异议请于签收之日起3日内提出, 否则视为已确认此对帐单余额, 谢谢</font></Data></Cell>
   </Row>
   ...
   <Row >
   {assign var=cross value=$arr_field_info|@count}
    <Cell {if $cross>1}ss:MergeAcross="{$cross-1}"{/if} ss:StyleID="s29"><Data ss:Type="String">
    {if $arr_main_value}
   ...
   <Row ss:AutoFitHeight="0">
    ...
    <Cell ss:StyleID="{if $align[1]=='right'}s25{else}s28{/if}"><Data ss:Type="{if $item.type=='right'}Number{else}String{/if}">{if $key2==''}{$field_value.$key|default:''|escape:'html'}{elseif $key3==''}{$field_value.$key1.$key2|default:''|escape:'html'}{else}{$field_value.$key1.$key2.$key3|default:''|escape:'html'}{/if}</Data></Cell>
    {/foreach}
   </Row>
   {/foreach}
  </Table>
