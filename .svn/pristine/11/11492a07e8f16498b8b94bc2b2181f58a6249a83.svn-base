...
$(function(){
	...
	*/
	...
	// 		}
	// 		//重新选中已出库的记录
	// 		for(var k in number_arr){
	// 			for(var t in _cache){
	// 				if(_cache[t]['lot'] == number_arr[k]){
	// 					_cache[t].isChecked=true;continue;
	// 				}
...
function getMadanList(){
	...
	for(var ii=0;ii<page;ii++) {
		...
		for(var j=0;j<5;j++) {
			...
			for(var k=0;k<20;k++) {
				...
				t.push("<div class='nn'>");
				//debugger;
				t.push("<input type='checkbox' name='sel[]' onclick='ckSel(this)' class='sel' disabled='disabled'/>");
				t.push("</div>");
	...
	for(var ib=0;ib<5;ib++){
		headArr.push("<div class='headDuan'><div class='j form-control'>卷号</div><div class='s form-control'>数量</div><div class='c form-control'>色号</div><div class='l form-control'>条码</div><div class='nn'></div></div>");
	}
...
function changeCache(i,p){
	if(!_cache)_cache=[];
	if(!_cache[p])_cache[p]={"id":'',"ruku2proId":'',"cnt":'',"cntFormat":'',"number":'',"lot":'',"seCode":'',"isChecked":false};
	var _t=_cache[p];
...
function getDataBynumber(num){
	...
	for(var t in _cache){
		...
		if(t>=pos && t<=(pos+99)){
			...
			if(_cache[t].readonly==1){
				document.getElementsByName('sel[]')[_pos].readOnly=true;
			}
		}
	}
}
...
function getHeji(){
	...
	//合计显示在页面上
	var hejiStr="总数："+"<font color='red'>"+heji.toFixed(2)+"</font>&nbsp;&nbsp;件数<font color='red'>："+ma+"</font>";
	$('#divHeji').html(hejiStr)
}
//返回给父窗口
function ok(){
	var o=getTotal();
	var strData=getSelect('id')+';'+getSelect('lot');
	console.log(getSelect('number'));

	var obj={"data":strData,"ok":1,"cnt":o.cnt,"cntJian":o.cntDuan};
...
.c {width:54px; float:left;border:0px solid #000; margin-top: 2px;}
...
.classHead {width:1125px;clear:both;text-align:left; font-weight:bold;}
.headDuan{width:225px; float:left;}
#divPage {width:1125px; clear:both; overflow: auto;}
#divBlock {width:225px; float:left;}
