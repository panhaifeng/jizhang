...
<div class="panel panel-info">
  <div class="panel-heading"><h3 class="panel-title" style="text-align:left;">{'织造计划编辑'}</h3></div>
  ...
  <div style="overflow:auto; border:0px solid #bce8f1; margin-bottom:10px; max-height:290px;">
    <div class="table-responsive" style="width:{$tbl_other_width|default:"100%"};">
      <table class="table table-condensed table-striped trRowMore" id='table_zhizao' key='zhizaoId[]' removeUrl='?controller={$smarty.get.controller}&action={$Remove2ZhizaoAjax|default:Remove2ZhizaoAjax}'>
        <thead>
          <tr>
            {foreach from=$zhizaoSon item=item key=key}
        ...
        <tbody>
          {foreach from=$rowzSon item=item1 key=key1}
          <tr class='trRow'>
            {foreach from=$zhizaoSon item=item key=key}
              {if $item.type!='bthidden'}
              <td>{webcontrol 
